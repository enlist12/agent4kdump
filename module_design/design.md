...
我们对于整个静态分析流程，首先是没有codequery的查看具体代码的设计。

然后对于静态分析的条件部分，我们需要建立一个tree，这个tree用来进行整个污点分析的污点的记录。
然后同时这个tree还需要能够回溯大模型的对话历史。

那么我需要一个结构，来存储整个流程的对话历史，然后这个tree的node记录具体的对话历史的点，从而达到回溯的效果
这里应该可以使用langchain的memory机制。但是具体的memory机制我应该去查询关于节点分叉回溯应该怎么丢弃不需要的分叉
或者尝试保留？？
...
这部分searchagent  他的职能
首先是根据崩溃报告来进行相似查询。
这里需要设计这个崩溃报告的指纹，或者其他的唯一性的信息。
# TODO： 请你填充到下面。

然后是关于RAG部分。我们对于之前保存的langchain memory 来做一个RAG。保存其中部分的思维链，然后让一个model来summary这个COT，然后将这个COT去输入到分析agent部分，从而来进行一个RAG查找。
这部分的话还可以增添关于Linux内核的模块的背景信息。这里可以做一点Linux内核的博客，网页的爬取工作。比如说，我们的crash
report 指出了这个错误发生在哪个驱动，那么就可以去搜索关于这个驱动的相关blog或者其他的技术报告，同时还应该标注这个Linux的版本问题，从而达到一个RAG的效果，为后面的分析agent提供一点背景知识。
