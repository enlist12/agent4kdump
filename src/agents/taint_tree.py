...
class MessageMemory:
    ...
    def create_checkpoint(
        ...
        checkpoint_id = f"msg_{uuid4().hex[:12]}"
    ...
    def run(
        ...
        while stack:
            ...
            if not node.taint_obj:
                ...
                self._warn(
                    memory, node, "taint_tree", f"Reached max_taint_steps={self.max_taint_steps}."
                )
                tree.mark(node.node_id, "done")
            else:
                ...
                if result is None:
                    ...
                    closed_without_children = True
                elif result.kind == "terminal":
                    if result.terminal_reason and node.taint_obj:
                        node.taint_obj.end = True
                        node.taint_obj.explain = self.join_text(
                            node.taint_obj.explain,
                            f"Stop: {result.terminal_reason}",
                        )
                    tree.mark(node.node_id, "done")
                    closed_without_children = True
                elif len(tree.nodes) >= self.max_tree_nodes:
                    ...
                    if result.next_obj:
                        ...
                        children = [tree.add_child(node.node_id, next_obj, node.checkpoint_id)]
                elif tree.branch_depth(node.node_id) < self.max_branch_depth:
                    capacity = self.max_tree_nodes - len(tree.nodes)
                    branches = sorted(result.branches, key=lambda item: item.priority)[
                    ...
                    for branch in branches[:capacity]:
                        ...
                        children.append(
                            tree.add_child(node.node_id, taint_obj, checkpoint_id, branch)
                        )

                if children:
                    tree.mark(node.node_id, "done")
                    stack.extend(reversed([child.node_id for child in children]))
                elif not closed_without_children:
                    reason = (
                        "Missing next_obj."
                        if result.kind == "single"
                        else "Missing branch children."
                    )
