...
def get_kdump_analysis_instance():
    ...
    if _KDUMP_ANALYSIS_INSTANCE is None:
        raise RuntimeError(
            "kdump_analysis instance not set. Call set_kdump_analysis_instance() first."
        )
    ...
    try:
        ...
        if not filter_gdb_command(gdb_command):
            return {
                "result": "error",
                "output": [
                    f"Command blocked for safety: '{gdb_command}'. Dangerous commands (shell escape, quit, etc.) are not allowed."
                ],
            }
