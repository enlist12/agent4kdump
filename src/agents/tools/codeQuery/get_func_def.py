...
def read_line_with_previous_part(file_path: str, line_no, proj_path):
    ...
    with open(os.path.join(proj_path, file_path), "r", errors="ignore") as f:
        ...
        while cur_line > 0:
            ...
            prev_line_ends = not prev_line.endswith(",\n") and prev_line.endswith(
                (";\n", "}\n", ")\n", "{\n")
            )
            if prev_line_ends:
                break

            prev_line_continues = prev_line.endswith(",\n") or prev_line.endswith('"\n')
            if (not prev_line_continues) and (prev_n_tabs, prev_n_blks) > (
                n_tabs,
                n_blks,
            ):  # if the previous line has more indentation
                break

            if (not prev_line_continues) and prev_n_tabs == n_tabs and prev_n_blks == n_blks:
                break
            cur_line -= 1
        ...
        # line_end = line_no - 1
        while line_end < len(lines):
            line_ends = not lines[line_end].endswith(",\n") and lines[line_end].endswith(
                (";\n", "}\n", ")\n", "{\n")
            )
            if line_ends:
                break
...
def read_func_first_part(file_path: str, line_no, proj_path):
    ...
    func_def, comment_start = __read_func(file_path, func_start_line, proj_path)
    ...
    with Cache(cache_dir + "/cache_func_read", size_limit=1 * 1024**3) as cache:
        ...
        with open(os.path.join(proj_path, file_path), "r", errors="ignore") as f:
            ...
            while line_start >= 0:
                ...
                candidate = lines[line_start]
                if (
                    candidate.strip()
                    and candidate[0] not in (" ", "\t", "{")
                    and not candidate.endswith(":")
                    and not is_comment_line(candidate)
                ):
                    break
                line_start -= 1
