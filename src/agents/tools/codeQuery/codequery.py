...
try:
    ...
    __HAS_DEPENDENCY = True
except (subprocess.CalledProcessError, FileNotFoundError, OSError, TimeoutError):
    __HAS_DEPENDENCY = False


def has_codequery_dependencies():
    return __HAS_DEPENDENCY


def create_cq_db(project_path):
    try:
        if not __HAS_DEPENDENCY:
            raise RuntimeError("Missing cscope, ctags, or codequery command.")

        if __exist_db_file(project_path):
            return

        cscope_files_path = os.path.join(project_path, "cscope.files")

        if not os.path.exists(cscope_files_path):
            with open(cscope_files_path, "w") as f:
                _run_command(
                    ...
                    stderr=sys.stderr,
                    check=True,
                    timeout=CODEQUERY_BUILD_TIMEOUT_SEC,
                )
    ...
    except (subprocess.CalledProcessError, TimeoutError, RuntimeError):
...
def __get_func_cq(project_path, function_name):
    ...
    if not __exist_db_file(project_path):
        ...
        if not __HAS_DEPENDENCY:
            logging.error("Error: Missing cscope, ctags, or codequery. Please install them first.")
    ...
    command = ["cqsearch", "-s", cqsearch_db, "-p", "2", "-u", "-e", "-t", function_name]
    res = []
...
def __get_struct_cq(project_path, struct_name):
    ...
    command = ["cqsearch", "-s", cqsearch_db, "-p", "3", "-u", "-e", "-t", struct_name]
    res = []
...
def __get_union_cq(project_path, union_name):
    ...
    command = ["cqsearch", "-s", cq_db, "-p", "1", "-u", "-e", "-t", union_name]
...
def __get_global_var_cq(project_path, var_name, grep_pattern="struct"):
    ...
    command = ["cqsearch", "-s", cqsearch_db, "-p", "1", "-u", "-e", "-t", var_name]
    res = []
...
def __get_caller_cq(project_path, function_name):
    ...
    command = [
        ...
        cqsearch_db,
        "-p",
        "6",  # 6: Functions calling this function
        "-u",
        "-e",
        "-t",
        function_name,
...
def __get_callee_cq(project_path, function_name):
    ...
    command = [
        ...
        cqsearch_db,
        "-p",
        "7",  # 7: Functions called by this function
        "-u",
        "-e",
        "-t",
        function_name,
